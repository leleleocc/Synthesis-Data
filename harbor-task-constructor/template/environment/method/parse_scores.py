...
def _quality_scan(
    ...
    faults: list[str],
    collected_full_workspaces: list[str],
) -> dict[str, Any]:
    quadrants = {
    ...
    if collected_full_workspaces:
        red_flags.append("collected_full_workspaces")
    ...
    return {
        ...
        "faults": len(faults),
        "collected_full_workspaces": collected_full_workspaces,
        "red_flags": red_flags,
...
def _next_action(
    *,
    ...
    if "collected_full_workspaces" in red_flags:
        return (
            "inspect_red_flags",
            "Stop fresh sampling. Move full mechanical-check Harbor jobs to /app/runtime/harbor-checks, keep only compact diagnostics and the result summary under /app/build/evidence, then rerun the parser. Reuse the current score round; do not rerun models or checks.",
        )
    if task_matches_final is not True:
        return (
            "verify_final_task",
            "Rerun with --final-task /app/build/task before deciding; only a matching task digest is current evidence.",
        )
...
def parse_round(round_dir: Path, final_task: Path | None = None) -> dict[str, Any]:
    ...
    quality_scan = _quality_scan(
        ...
        arms,
        faults,
        _collected_full_workspaces(round_dir),
    )
