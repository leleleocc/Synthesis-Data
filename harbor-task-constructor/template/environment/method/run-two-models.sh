...
task_root="${SOP_TASK_ROOT:-/app/build/task}"
evidence_root="${SOP_EVIDENCE_ROOT:-/app/build/evidence}"
runtime_evidence_root="${SOP_RUNTIME_EVIDENCE_ROOT:-/app/runtime/harbor-evidence}"
retention_module="${SOP_RETENTION_MODULE:-$(dirname "$0")/internal/evidence_retention.py}"
parser_path="${SOP_PARSER_PATH:-$(dirname "$0")/parse_scores.py}"
harbor_python="${SOP_HARBOR_PYTHON:-python3}"
attempts=2
attempts_supplied=0
concurrency=1
...
declare -a child_runtime_runs=()
declare -a arm_run_dirs=()
declare -a runtime_arm_run_dirs=()
credential_dir=""
allocation_lock_dir=""
...
cleanup_children() {
  ...
  for index in "${!child_pids[@]}"; do
    ...
    printf '%s\n' "$child_status" > "${child_runtime_runs[$index]}/exit-code"
  done
  child_pids=()
  child_runs=()
  child_runtime_runs=()
...
cleanup() {
  ...
  if [[ -n "$runtime_round_dir" ]]; then
    rm -f -- "$runtime_round_dir/.active"
  fi
  if [[ "$mode" == "regrade" && "$publication_succeeded" == 1 && "$runtime_retention_completed" == 0 ]]; then
    if retain_runtime "$source_round"; then
      runtime_retention_completed=1
    fi
  fi
  release_allocation_lock
...
if config.verifier.environment_mode != VerifierEnvironmentMode.SEPARATE:
    fail(
        ...
        "/app/method/run-two-models.sh both to create a reusable real rollout. "
...
regrade_sources_json() {
  "$harbor_python" - "$1" "$2" "$3" "${4:-strict}" "$evidence_root" <<'PY'
import json
import re
import sys
from pathlib import Path
...
collected_root = Path(sys.argv[5])
...
if not rounds and selection == "strict":
    fail(
        ...
        "Next action: run /app/method/run-two-models.sh both to create a reusable "
...
retain_runtime() {
  local keep_round="$1"
  local -a keep_args=()
  [[ -z "$keep_round" ]] || keep_args=(--keep-round "$keep_round")
  "$harbor_python" "$retention_module" retain --runtime-root "$runtime_evidence_root" ${keep_args[@]+"${keep_args[@]}"}
}

publish_current_round() {
  local index publication_status=0
  local -a include_agent_args=() source_round_args=()
  publication_attempted=1
  if [[ "$mode" == "regrade" ]]; then
    source_round_args=(--source-round "${source_round##*/}")
  else
    include_agent_args=(--include-agent)
  fi
  for index in "${!arm_run_dirs[@]}"; do
    "$harbor_python" "$retention_module" publish-run \
      --runtime-run "${runtime_arm_run_dirs[$index]}" \
      --collected-run "${arm_run_dirs[$index]}" \
      ${include_agent_args[@]+"${include_agent_args[@]}"} || publication_status=1
  done
  if (( publication_status != 0 )); then
    echo "could not publish all compact Harbor runs; runtime diagnostics retained" >&2
    return 1
  fi
  if "$harbor_python" "$retention_module" write-round \
    --collected-round "$round_dir" --kind "$round_kind" ${source_round_args[@]+"${source_round_args[@]}"}; then
    publication_succeeded=1
  else
    return 1
  fi
...
task_name="$(python3 - "$task_state" <<'PY'
import json
import sys
print(json.loads(sys.argv[1])["task_name"])
PY
)"
task_digest="$(python3 - "$task_state" <<'PY'
import json
import sys
print(json.loads(sys.argv[1])["task_digest"])
PY
)"
rollout_input_digest="$(python3 - "$task_state" <<'PY'
import json
import sys
...
if [[ "$mode" == "regrade" ]]; then
  round_kind="regrade"
  regrade_source_json="$(regrade_sources_json "$runtime_evidence_root" "$task_name" "$rollout_input_digest")"
...
previous_source_json="$(regrade_sources_json "$runtime_evidence_root" "$task_name" "$rollout_input_digest" retained)"
...
if [[ -n "$round_number" ]]; then
  ...
  runtime_round_dir="$runtime_evidence_root/round-$formatted_round"
  if [[ ! -d "$round_dir" || ! -d "$runtime_round_dir" ]]; then
    echo "requested current-life runtime round does not exist: $runtime_round_dir" >&2
    exit 2
  fi
fi

allocation_barrier_for_test
mkdir -p "$runtime_evidence_root"

if [[ -n "$round_number" ]]; then
  if ! rollout_lock_matches "$runtime_round_dir/rollout.lock" "$task_digest" "$rollout_input_digest"; then
    echo "rollout identity does not match the current task" >&2
    exit 1
  fi
  : > "$runtime_round_dir/.active"
  retain_runtime "$previous_source_round"
else
  if [[ "$mode" == "regrade" ]]; then
    retain_runtime "$source_round"
  else
    retain_runtime "$previous_source_round"
  fi
  round_dir="$(claim_next_directory "$evidence_root" "round-")"
  claimed_round_dir="$round_dir"
  runtime_round_dir="$runtime_evidence_root/${round_dir##*/}"
  mkdir "$runtime_round_dir"
  : > "$runtime_round_dir/.active"
fi
...
for arm in "${arms[@]}"; do
  ...
  if arm_run_dir="$(claim_next_directory "$arm_root" "run-")"; then
    ...
    runtime_arm_run_dir="$runtime_round_dir/$arm/${arm_run_dir##*/}"
    mkdir -p "$runtime_arm_run_dir/jobs"
    runtime_arm_run_dirs+=("$runtime_arm_run_dir")
  else
    rollback_empty_claims
    exit 1
  fi
...
if [[ -z "$round_number" && "$mode" != "regrade" ]]; then
  if ! write_rollout_lock "$runtime_round_dir" "$task_digest" "$rollout_input_digest"; then
    rollback_empty_claims
    exit 1
  fi
fi
...
launch_arm() {
  local arm="$1" index="$2" model effort host arm_run_dir runtime_arm_run_dir
  arm_run_dir="${arm_run_dirs[$index]}"
  runtime_arm_run_dir="${runtime_arm_run_dirs[$index]}"
  local -a command=()
  if [[ "$mode" == "regrade" ]]; then
    command=(
      ...
      --jobs-dir "$runtime_arm_run_dir/jobs"
    )
  else
    ...
    command+=(--allow-agent-host "$host" --jobs-dir "$runtime_arm_run_dir/jobs")
  fi
  (
    scrub_child_environment
    exec "${command[@]}"
  ...
  child_runtime_runs+=("$runtime_arm_run_dir")
}
...
wait_for_children() {
  ...
  while [[ -n "${child_pids[0]:-}" ]]; do
    ...
    printf '%s\n' "$child_status" > "${child_runtime_runs[0]}/exit-code"
    if (( child_status != 0 )); then
      status="$child_status"
    fi
    child_pids=("${child_pids[@]:1}")
    child_runs=("${child_runs[@]:1}")
    child_runtime_runs=("${child_runtime_runs[@]:1}")
    write_active_runs_for_test
  done
  return "$status"
}
...
if [[ "$mode" == "regrade" ]]; then
  retain_runtime "$source_round"
  runtime_retention_completed=1
elif (( harbor_status == 0 )); then
  complete_source_json="$(regrade_sources_json "$runtime_evidence_root" "$task_name" "$rollout_input_digest" complete)"
  complete_source_round="$(json_source_round "$complete_source_json")"
  if [[ "${complete_source_round##*/}" == "${runtime_round_dir##*/}" ]]; then
    remember_complete_source "$complete_source_json"
    retain_runtime "$runtime_round_dir"
  fi
fi
