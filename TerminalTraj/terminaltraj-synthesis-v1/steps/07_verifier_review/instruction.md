# Phase 07: per-candidate verifier review

## ROLE

You are phase 07 only. Run independently from the verifier author. You own
`/synthesis/state/07_verifier_review.json` and minimal edits under
...
- Do not write `/synthesis/state/06_verifier.json` or any other prior
  `/synthesis/state/*.json`. Phase 06 sealed that mapping. This phase writes
  only `/synthesis/state/07_verifier_review.json`. If on-disk tests cover
...
- Do not start phase 08 work in this turn.
...
Hard gate: `/synthesis/state/07_verifier_review.json`. Stopping without that
file fails the phase.

```bash
python /opt/terminaltraj/scripts/target_spec.py ensure-handoff 07_verifier_review
```

Write `/synthesis/state/07_verifier_review.json` atomically:
...
Phase 08 assembles the **final** `tests/` trees from disk after these edits.
...
* Fix `reward.txt` handling, `set -uo pipefail` (no `-e`), executable bit,
  `bash -n` failures
* Rewrite a pytest / `apt-get` / `uvx` verifier onto this candidate's
  `oracle_tools` from `04_task_design.json` plus POSIX bash. Verify-time
  package installs are a fail, not a style note. Do not edit overlays.
...
* Edit `instruction.md` or `task.toml` to match a weak test (instruction is
  owned by phases 04-05; only fix an instruction if it is a pure typo that
  blocks a correct test, and record that edit explicitly)
* Edit `environment/` or sealed prior state JSON
* Write or rewrite `/synthesis/state/06_verifier.json` (or any prior state
  JSON). The verifier mapping is sealed; this phase owns only
  `07_verifier_review.json` plus local `tests/` edits
...
again. Do not modify sealed `06_verifier.json` (phase 08 verifies its seal).
...
For every verifier in step 06, give `"pass"`, `"fail"`, or `"not_applicable"`
...
installs packages at verify time (`apt-get`, `pip install`, `uvx`,
`curl | sh`) or calls a binary not in `oracle_tools` / 03 `entrypoints`
/ bash. Rewrite onto the listed tools; do not invent pytest.
