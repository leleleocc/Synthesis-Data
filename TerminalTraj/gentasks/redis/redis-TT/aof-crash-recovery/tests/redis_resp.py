...
class Redis:
    ...
    def info(self, section: str) -> dict[str, str]:
        raw = to_str(self.execute_ok("INFO", section)) or ""
        out: dict[str, str] = {}
    ...
    def keys(self) -> list[str]:
        ...
        cursor: object = "0"
        while True:
            reply = self.execute_ok("SCAN", cursor, "COUNT", "1000")
            if not isinstance(reply, list) or len(reply) != 2:
                raise RuntimeError(f"SCAN returned {reply!r}")
            cursor = to_str(reply[0])
            batch = reply[1] or []
            for item in batch:
                found.append(to_str(item) or "")
            if cursor == "0":
                break
        return found
