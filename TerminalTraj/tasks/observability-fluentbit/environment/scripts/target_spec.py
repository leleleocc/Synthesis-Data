...
def check_design(
    ...
    """Phase 04: each candidate is instruction.md + task.toml + overlay environment/."""
    errors: list[str] = []
    target = spec["target"]
...
def _verifier_runtime_errors(cid: Any, tests_dir: Path, oracle_tools: list[str] | None = None) -> list[str]:
    """Phase 06/07 tests must stay offline; extras come from phase-04 oracle_tools."""
    errors: list[str] = []
    ...
    for path in tests_dir.rglob("*"):
        ...
        if _VERIFY_INSTALL_RE.search(text):
            errors.append(
                ...
                "phase 04 must put observation tools in the overlay / oracle_tools"
            )
...
def check_verifiers(
    ...
    """Phase 06: one Harbor-style tests/test.sh per approved candidate."""
    errors: list[str] = []
...
def check_verifier_review(state: dict[str, Any], review: dict[str, Any], candidates_dir: Path) -> list[str]:
    """Phase 07: rubric verdicts per verifier, syntax re-checked."""
    errors: list[str] = []
    candidates_dir = Path(candidates_dir)
...
def check_task_toml_assembled(text: str, slug: str, env_state: dict[str, Any]) -> list[str]:
    """Packaged task.toml must stay a valid single-step Harbor file (from phase 04/05)."""
    import tomllib
...
def _toml_core_equal(left_text: str, right_text: str, slug: str) -> list[str]:
    ...
    if l2 != r2:
        errors.append(
            f"task.toml must be the phase-04/05 draft aside from [task].name "
...
def check_package(
    ...
    for item in tasks:
        ...
        # task.toml: phase-04/05 draft with name/slug alignment only
...
PHASE_HANDOFFS: dict[str, dict[str, Any]] = {
    ...
    "05_task_review": {
        "path": "/synthesis/state/05_task_review.json",
        "skeleton": {"reviews": [], "approved_candidate_ids": []},
    },
    "06_verifier": {
        "path": "/synthesis/state/06_verifier.json",
        "skeleton": {"verifiers": []},
    },
    "07_verifier_review": {
        "path": "/synthesis/state/07_verifier_review.json",
        "skeleton": {"reviews": [], "approved_candidate_ids": []},
    },
}
...
def main(argv: list[str]) -> int:
    if len(argv) < 2:
        print(
            ...
            "check-package SPEC STATE03 DESIGN04 STATE06 APPROVED_STATE ENV_DIR CANDIDATES_DIR GENERATED_DIR | "
    ...
    if command == "check-package":
        # SPEC STATE03 DESIGN04 STATE06 APPROVED_STATE ENV_DIR CANDIDATES_DIR GENERATED_DIR
        if len(argv) < 10:
            print(
                "check-package requires: SPEC STATE03 DESIGN04 STATE06 "
