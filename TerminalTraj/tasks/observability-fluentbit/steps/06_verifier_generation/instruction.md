# Phase 06: per-candidate verifier generation

## ROLE

You are phase 06 only. Run independently from files on disk; do not rely on
prior conversation. You own `candidates/<id>/tests/**` and
`/synthesis/state/06_verifier.json` only. You do not edit candidate
...
- Do not generate verifiers for candidates that did not pass step 05. If the
...
- Do not start phase 07/08 work in this turn.
...
- `/synthesis/state/04_task_design.json` (per-candidate `oracle_tools`)
...
- `/opt/terminaltraj/skills/harbor-task-creator/SKILL.md` and
  ...
  Copy the reward-channel shape, not their toy assertions. Test-only
  dependencies may be installed by `test.sh`; keep task/runtime dependencies
  in the base environment or candidate overlay. Pin versions and do not use
  `curl | sh` or unbounded installs.

## HANDOFF

Hard gate: for every approved candidate id in
`05_task_review.json.approved_candidate_ids`:
...
3. one record for that id inside `/synthesis/state/06_verifier.json`

Plus the phase handoff itself:

4. `/synthesis/state/06_verifier.json`

Land the handoff **before** long ground-truth probes:

```bash
python /opt/terminaltraj/scripts/target_spec.py ensure-handoff 06_verifier
```
...
Write `/synthesis/state/06_verifier.json` atomically:
...
on (including any tolerance and why), and is what step 07 uses to judge
...
Run `ensure-handoff 06_verifier` immediately. For every approved id, write a
...
- It is deterministic and bounded in time. `test.sh` may install
  test-only dependencies with pinned versions, but installation failure must
  produce reward `0`; do not use `curl | sh` or unpinned `latest` packages.
  Task/runtime dependencies still belong in the base environment or overlay.
- Tools available before `test.sh` are POSIX `bash` / coreutils, this
  candidate's `oracle_tools` from `04_task_design.json`, and 03 `entrypoints`.
  A dependency installed successfully by `test.sh` may then be invoked by the
  verifier; record its exact package and version in `test.sh` and the review
  explanation.
  If `oracle_tools` includes `python3`, use stdlib (`json`, `hashlib`,
  `pathlib`) unless a pinned test dependency is explicitly installed.
...
- Use pytest, CTRF, or another test framework only when `test.sh` installs a
  pinned version before invoking it. Otherwise use bash plus the listed
  `oracle_tools` and 03 `entrypoints`.
...
it from `checked_requirements` so step 07 can reject the candidate.

Replace each skeleton with a real verifier, keep helper/golden files under
that `tests/` tree, and fill the matching `06_verifier.json` record.
...
ids = json.loads(Path("/synthesis/state/05_task_review.json").read_text())["approved_candidate_ids"]
...
Fix anything reported, then stop. Do not start phase 07/08 work in this turn.
