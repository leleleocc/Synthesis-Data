...
require_handoff 05_task_review
S=/synthesis/state/05_task_review.json
...
python /opt/terminaltraj/scripts/target_spec.py check-review \
  ...
  /synthesis/state/04_task_design.json "$S" \
  /synthesis/output/candidates \
  || fail "candidate reviews are incomplete or inconsistent"
for id in $(python -c 'import json; print("\n".join(json.load(open("/synthesis/state/05_task_review.json"))["approved_candidate_ids"]))'); do
...
PY
seal_state "$S" /synthesis/state/.integrity/05_task_review.sha256
