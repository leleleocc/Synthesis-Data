...
def check_step04_spec_gate() -> None:
    ...
    if "target.tags" not in instruction and "target_spec.target" not in instruction:
        error("step 04 instruction does not bind vocabulary fields to target_spec")
    if "task.toml" not in instruction or "instruction.md" not in instruction:
        error("step 04 instruction must require per-candidate instruction.md and task.toml")
    if "environment/" not in instruction:
        error("step 04 instruction must require a per-candidate overlay environment/")
    if "pure ASCII" not in instruction and "pure ascii" not in instruction.lower():
        error("step 04 instruction must require generated instruction.md/task.toml to be ASCII")
    if 'network_mode = "public"' not in instruction:
        error("step 04 instruction must default [environment].network_mode to public")
    if "max_candidates" not in instruction:
        error("step 04 instruction must bind the matrix to generation.max_candidates")
    for needle, why in (
        ...
        ("candidate matrix", "diversity matrix section"),
        ...
        ("oracle_tools", "phase 04 must declare observation tools for the later verifier"),
        ('"tags"', "phase 04 index uses tags lists, not task_types"),
        ('"subdomains"', "phase 04 index uses subdomains lists"),
        ('"oracle_types"', "phase 04 index uses oracle_types lists, not a single oracle_type"),
        ("taxonomy.md", "phase 04 must classify from the Terminal-Bench taxonomy"),
    ):
        if needle not in instruction:
            error(f"step 04 instruction lacks {why} ({needle!r})")
    if "target_spec.py check-design" not in test:
        error("step 04 test does not check design against target_spec")
    if "task_contract" in instruction or "observable_outcome" in instruction:
        error("step 04 instruction still requires the old heavy design JSON fields")
    if "instruction_path" in instruction or "task_toml_path" in instruction:
        error("step 04 instruction still requires redundant path fields in the design index")
    if '"task_types"' in instruction:
        error("step 04 instruction still uses task_types; use tags")
    stripped = instruction.replace('"oracle_types"', "")
    if '"oracle_type"' in stripped:
        error("step 04 instruction still uses singular oracle_type JSON keys")
    if "max_query_candidates" in instruction or "max_tasks" in instruction:
        error("step 04 instruction still references removed generation budget fields")
    ...
    if "harbor-task-creator" not in instruction:
        error("step 04 instruction must tell the agent to synthesize task.toml from harbor-task-creator")
    if "/opt/terminaltraj/skills/harbor-task-creator" not in instruction:
        error("step 04 instruction must point at /opt/terminaltraj/skills/harbor-task-creator")
    if "example-tasks.md" not in instruction:
        error("step 04 instruction must point at harbor-task-creator example-tasks.md")
    if "very simple demo" not in instruction:
        error("step 04 instruction must label skill examples as very simple demos")


def check_multi_candidate_gates() -> None:
    review_instruction = read(ROOT / "steps" / "05_task_review" / "instruction.md")
    review_test = read(ROOT / "steps" / "05_task_review" / "tests" / "test.sh")
    verifier_instruction = read(ROOT / "steps" / "06_verifier_generation" / "instruction.md")
    verifier_test = read(ROOT / "steps" / "06_verifier_generation" / "tests" / "test.sh")
    package_instruction = read(ROOT / "steps" / "08_package" / "instruction.md")
    package_test = read(ROOT / "steps" / "08_package" / "tests" / "test.sh")
    checks = (
        (review_instruction, "approved_candidate_ids", "step 05 prompt lacks approved candidate list"),
        (review_instruction, "pure ASCII", "step 05 prompt must require generated instruction.md to stay ASCII"),
        (review_instruction, "env_build=failed", "step 05 prompt must reject failed overlays"),
        (review_instruction, "Do not write `/synthesis/state/04_task_design.json`", "step 05 prompt must forbid rewriting sealed 04_task_design.json"),
        (review_test, "target_spec.py check-review", "step 05 test lacks review contract check"),
        (verifier_instruction.lower(), "for every approved candidate", "step 06 prompt is not multi-candidate"),
        (verifier_test, "target_spec.py check-verifiers", "step 06 test lacks the shared verifier contract check"),
        (read(ROOT / "steps" / "07_verifier_review" / "tests" / "test.sh"), "target_spec.py check-verifier-review", "step 07 test lacks the shared verifier review check"),
        (read(ROOT / "steps" / "07_verifier_review" / "instruction.md"), "Too strict", "step 07 prompt lacks the too-loose/too-strict calibration lists"),
        (verifier_instruction, "Too strict", "step 06 prompt lacks the too-loose/too-strict calibration lists"),
        (verifier_instruction, "harbor-task-creator", "step 06 prompt must point at harbor-task-creator for reward-channel wiring"),
        (verifier_instruction, "very simple demo", "step 06 prompt must label skill examples as very simple demos"),
        (verifier_instruction, "example-tasks.md", "step 06 prompt must point at skill example walkthroughs"),
        (verifier_instruction, "oracle_tools", "step 06 prompt must use phase-04 oracle_tools"),
        (read(ROOT / "steps" / "07_verifier_review" / "instruction.md"), "oracle_tools", "step 07 prompt must rewrite verify-time installs onto oracle_tools"),
        (read(ROOT / "steps" / "07_verifier_review" / "instruction.md"), "Do not write `/synthesis/state/06_verifier.json`", "step 07 prompt must forbid rewriting sealed 06_verifier.json"),
        (package_instruction, "generated_task/<slug>", "step 08 prompt is not multi-package"),
        (package_instruction, "merged with", "step 08 prompt must merge base with overlay"),
        (package_test, "manifest.json", "step 08 test lacks package manifest check"),
    )
    for text, needle, message in checks:
        if needle not in text:
            error(message)
    if "max_tasks" in review_instruction:
        error("step 05 prompt still caps approvals with max_tasks")


def check_step08_package_gate() -> None:
    test = read(ROOT / "steps" / "08_package" / "tests" / "test.sh")
    instruction = read(ROOT / "steps" / "08_package" / "instruction.md")
    if "manifest.json" not in test:
        error("step 08 test missing manifest.json check")
    if "target_spec.py check-package" not in test:
        error("step 08 test must call check-package")
    if "output/environment" not in test:
        error("step 08 test must pass the synthesized environment path")
    if "04_task_design.json" not in test:
        error("step 08 test must pass phase-04 design state into check-package")
    if "README.md" not in instruction:
        error("step 08 instruction must require per-package README.md")
    if "task.toml" not in instruction or "instruction.md" not in instruction:
        error("step 08 instruction must assemble task.toml and instruction.md from candidates")
    if "Do not redesign" not in instruction and "assemble only" not in instruction.lower():
        error("step 08 instruction must state assemble-only (no redesign)")
    if "check-environment" in test:
        error("step 08 test must not re-run check-environment")
...
def check_task_toml_steps() -> None:
    ...
    # sealed base (03) and optional overlay images (04) need a real budget.
    for name, minimum in (("03_environment", 3600.0), ("04_task_design", 3600.0)):
    ...
    eight = by_name.get("08_package") or {}
    if eight.get("min_reward") != 1.0:
        error("08_package needs min_reward = 1.0 so a failed assemble fails the trial")
...
def check_phase_handoff_gates() -> None:
    ...
    step_phases = {
        ...
        "04_task_design": "04_task_design",
        "05_task_review": "05_task_review",
        "06_verifier_generation": "06_verifier",
        "07_verifier_review": "07_verifier_review",
    }
    helpers = read(ROOT / "tests" / "helpers.sh")
    ...
    pkg_instruction = read(ROOT / "steps" / "08_package" / "instruction.md")
    for phase in ("03_environment", "04_task_design", "06_verifier"):
        if f"require-handoff {phase}" not in pkg_instruction:
            error(f"08_package/instruction.md must require-handoff {phase}")
...
def main() -> int:
    ...
    check_step04_spec_gate()
    check_multi_candidate_gates()
    check_step08_package_gate()
