# Phase 04: task design and query generation

## ROLE

You are phase 04 only. Run this phase independently from files on disk; do not
...
- Do not start phase 05/06 work in this turn.
...
Hard gate: `/synthesis/state/04_task_design.json`. Stopping without the state
file fails the phase even if some `instruction.md` files exist.

Land the index before writing many candidates:

```bash
python /opt/terminaltraj/scripts/target_spec.py ensure-handoff 04_task_design
```
...
build did not succeed. Phase 05 will not approve `failed`.

`oracle_tools` is the observation surface for phase 06: argv0 names that
...
phase-03 or phase-06 install.

### Step 02: build a candidate matrix (diversity)

Produce **exactly** `generation.max_candidates` candidates. Before writing any
instruction, lay out a matrix so the set is diverse, not N variants of one
idea. Each candidate takes **lists**, not a single token:

- `tags`, `oracle_types`, `domains`, `subdomains`, `languages` are non-empty
  unique subsets of the matching `target.*` lists. When a target list has two
  or more values, that candidate field must contain **at least two**. A
  one-tag repair or a one-oracle file dump is too thin for this factory.
- Cover every `target.oracle_types` value at least once across the set when
  the environment can host it. Spread remaining slots across `target.tags`
  and `target.subdomains`; do not emit N copies of the same tag pair.
- Combine tags that actually interact in the job (`configuration`+`repair`,
  `build`+`optimization`, `debugging`+`persistence`), not a laundry list.
  The instruction goal must name the interacting pieces.
...
- No two candidates share the same primary entrypoint **and** the same output
  shape (for example two "convert X to CSV with csvcut" tasks).
- Use different fixtures or different quirks of the same fixture across
  candidates; at most two candidates may share a primary input file.
- Vary the scenario angle: migration, incident/cleanup, onboarding hand-off,
  report for a stakeholder, audit/verification, integration with a downstream
  system, reproduction of a published number. A candidate is a job someone
  would actually be asked to do, with a plausible reason.
- Vary the output form: files with a normative schema, a directory layout, a
  modified config or source tree, a running process/port, a short answer file.
...
its tests/, solution/, or Dockerfile recipes in this phase (phase 06 owns
...
it even if empty of files). Phase 08 merges this tree over the sealed base
(overlay wins on the same relative path).

After the instruction exists, decide how phase 06 will observe success
...
- Overlay build failure does not rewrite the sealed base. Record
  `env_build=failed`; phase 05 will reject that candidate.
...
Fix anything it reports, then stop. Do not start phase 05/06 work in this turn.
