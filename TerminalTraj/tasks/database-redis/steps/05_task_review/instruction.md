# Phase 05: per-candidate task review

## ROLE

You are phase 05 only. Run this phase independently from files on disk; do not
infer requirements from chat history. You own `/synthesis/state/05_task_review.json`
...
design pass. Phase 06 must implement tests against the **final** instruction on
disk.
...
- Do not write `/synthesis/state/04_task_design.json` or any other prior
  `/synthesis/state/*.json`. Phase 04 sealed that index. This phase writes
  only `/synthesis/state/05_task_review.json`.
...
- Do not start phase 06+ work in this turn.
...
Hard gate: `/synthesis/state/05_task_review.json`. Stopping without that file
fails the phase.

Land the skeleton before long scoring:

```bash
python /opt/terminaltraj/scripts/target_spec.py ensure-handoff 05_task_review
```

Write `/synthesis/state/05_task_review.json` atomically. Final shape:
...
Field notes: one review per phase-04 candidate; `approved` matches
...
* `task.toml`: align `[task].description` / keywords with the instruction;
  ...
  metadata consistent with the phase-04 index; keep the file ASCII
* overlay `environment/`: remove a leaking fixture; add a missing COPY source;
  ...
  tools (`oracle_tools` lives in sealed phase-04 state).
...
* Edit `/synthesis/output/environment/`, sealed prior state JSON, or other
  candidates' unrelated files
* Write or rewrite `/synthesis/state/04_task_design.json` (or any prior
...
Fill every field of `/synthesis/state/05_task_review.json` from the scored,
edited candidates.

## GATE

```bash
python /opt/terminaltraj/scripts/target_spec.py require-handoff 05_task_review
