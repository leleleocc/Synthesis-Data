...
terminaltraj-synthesis/
  ...
  environment/skills/harbor-task-creator/  # Harbor wiring skill (phases 03/05/07; examples are simple demos)
  tests/helpers.sh
  scripts/check_contracts.py
  steps/
    01_repo_profile/ ... 09_package/
...
`junior_time_estimate_min` (>= 2x that estimate). Step 05 derives
...
Step 04 writes a Simula plan of **exactly** `generation.max_candidates` slots
(Global node-sets, Local surface variants, in-band Complexification). Slot
vocab lists (`tags`, `domains`, `subdomains`, `languages`, `oracle_types`)
are unique subsets of those target lists. When a target list has two or more
values, that field must contain at least two (a one-tag or one-oracle job is
too thin). Local **allows** the same tag pair across slots of one
`global_id`. Step 05 instantiates Harbor files from those slots (same ids,
vocab equals the slot). Step 06 reviews every candidate against the
instruction-level part of the Terminal-Bench task rubric: seven scores
(proposal rubric plus spec alignment) that must all be at least `0.7`, and
nine pass/fail/not_applicable checks named after the CI rubric criteria
...
04 simula plan           # Global / Local / Complexity slots; paper only
05 task design           # consume slots: instructions first, then toml + overlay + oracle_tools
06 task review           # review instruction; minimal in-place fixes; approve all that qualify
07 verifier generation   # per approved candidate: tests/** only
08 verifier review       # review tests; minimal in-place fixes under tests/ allowed
09 package               # assemble: instruction/task.toml/tests + base env merged with overlay
```

Ownership of artifacts:

- **Environment base (03):** only `/synthesis/output/environment/` (frozen afterwards)
- **Simula plan (04):** `/synthesis/state/04_simula_plan.json` only
- **Task (05/06):** `candidates/<id>/instruction.md`, `task.toml`, overlay `environment/`
- **Tests (07/08):** only `candidates/<id>/tests/**`
- **Package (09):** merge those pieces into `generated_task/<slug>/` and add `README.md`
...
(`01_repo_profile` ... `08_verifier_review`). Agents must land a skeleton first
...
`check-environment` or `docker build`. Step 05 may docker-build a **candidate
overlay** when that overlay replaces Dockerfile or setup; overlay failure
rejects that candidate in step 06 and never rewrites the base.

Harbor honors per-step `[steps.agent].timeout_sec` (see `StepConfig.agent` and
`MultiStepTrial._step_agent_timeout_sec`). Steps 03 and 05 set this to 3600 so
...
including 09, sets `min_reward = 1.0` so a failed assemble fails the trial
(Harbor `mean` would otherwise hide an 09 zero).

Phase 09 writes `/synthesis/output/generated_task/<slug>/` per survivor:

```text
instruction.md  # from 05, after 06 edits
task.toml       # from 05 (name/slug alignment only)
environment/    # 03 base merged with 05 overlay (overlay wins on the same path)
tests/          # from 07, after 08 edits
...
preinstall a verifier stack. Phase 05, which already knows the task, lists
`oracle_tools` and installs any missing observation binary in that
candidate's overlay (`setup-overlay.sh` preferred). Phase 07 `test.sh` may
install pinned test-only dependencies itself; task/runtime dependencies still
belong in the base environment or candidate overlay. Diversity that cannot
share one image (a planted bug, a broken redis.conf, extra logs, extra CLIs)
lives in `candidates/<id>/environment/`.
...
`harbor run -p generated_task/<slug>`. Phases 05 and 06 must write those
...
or inputs rather than expecting an automatic return to phase 05/07. Step 02
...
image builds. Step 08 reviews verifiers statically (syntax, coverage of the
