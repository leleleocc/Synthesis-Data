# Phase 09: package Harbor tasks (assemble only)

## ROLE

You are phase 09 only. Assemble only. Run independently from files on disk.
...
- Environment (03 sealed base + 05 overlay): `/synthesis/output/environment/`
  and `candidates/<id>/environment/`
- Task (05, reviewed by 06): `candidates/<id>/instruction.md` and `task.toml`
- Tests (07, reviewed by 08): `candidates/<id>/tests/**`
...
- `/synthesis/state/08_verifier_review.json` (if present)
...
Use `state/08_verifier_review.json.approved_candidate_ids` when that file
exists and lists ids; otherwise use
`state/06_task_review.json.approved_candidate_ids`. Package every id in that
list (order preserved). Count must be >= 1.

### Step 02: copy with the five rules

1. `instruction.md`: exact copy from the candidate **after** phase 06
   (including any minimal review edits). Phase 09 does not edit it further.
   It must already be pure ASCII (phases 05-06 own that); Harbor host-reads
...
2. `tests/`: exact recursive copy of the candidate `tests/` tree **after**
   phase 08 (including any minimal review edits). Keep `test.sh` executable.
...
4. `task.toml`: start from the candidate `task.toml` **after** phase 06.
   The only phase-09 edit is aligning identity with the package directory:
...
5. `README.md` (required): plain text or markdown for humans only. Include at
   ...
   environment is phase-03 base merged with the phase-05 overlay, tests came
   from phase 07-08, and instruction from phases 05-06. No solution, no hidden
   answers.
...
Phase 09 has no new state JSON; its handoff is the package tree. Still confirm
...
test -f /synthesis/state/08_verifier_review.json \
...
If `08_verifier_review.json` is missing, pass `06_task_review.json` in its
