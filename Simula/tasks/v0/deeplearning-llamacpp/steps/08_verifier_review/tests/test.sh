...
require_handoff 08_verifier_review
S=/synthesis/state/08_verifier_review.json
require_json_keys "$S" reviews approved_candidate_ids
python /opt/terminaltraj/scripts/target_spec.py check-verifier-review \
  /synthesis/state/07_verifier.json "$S" /synthesis/output/candidates \
  || fail "verifier review records do not satisfy the phase 08 contract"
...
PY
seal_state "$S" /synthesis/state/.integrity/08_verifier_review.sha256
