...
- Bake `pytest==9.1.1` and `pytest-json-ctrf==0.5.2` into the image that runs the verifier (`environment/Dockerfile` when inline; `tests/Dockerfile` when separate). Do not install packages in `test.sh`. Inline pytest in the agent image is required, not a leak.
...
# pytest==9.1.1 and pytest-json-ctrf==0.5.2 are already in the verifier image.
...
- Never install packages in test.sh. Bake `pytest==9.1.1` and `pytest-json-ctrf==0.5.2` into the image that runs the verifier.
...
- Pin dependency versions in the Dockerfile, not in test.sh
...
9. **test_deps_in_image** — Test-only dependencies (`pytest==9.1.1`, `pytest-json-ctrf==0.5.2`) are baked into the image that runs the verifier, not installed in test.sh
...
**GPU tasks:** For ML/CUDA workloads. Set `gpus` and `gpu_types` in `[environment]`. Daytona is the only backend and allocates GPUs only for a single-container Dockerfile task. `gpu_types` must be chosen from `H100`, `H200`, `RTX-PRO-6000`, `RTX-4090`, `RTX-5090`, and `MI355X`. Run with `-e daytona`.
...
| Unpinned dependencies | Builds break when packages update | Pin every version: `flask==3.1.0`, `pytest==9.1.1` |
...
| Requesting GPUs on compose or a non-Daytona type | Daytona rejects compose GPU tasks and unknown `gpu_types` | Use a single-container Dockerfile, `-e daytona`, and a type from `H100`, `H200`, `RTX-PRO-6000`, `RTX-4090`, `RTX-5090`, `MI355X` |
