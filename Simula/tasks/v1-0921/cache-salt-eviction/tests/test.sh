...
setpriv --reuid nobody --regid nogroup --clear-groups --no-new-privs \
  python3 -m pytest --ctrf /tmp/ctrf/ctrf.json /tests/test_prefix_hits.py -rA
status=$?
set -e
...
if [[ $status -ne 0 ]]; then
  echo "hash isolation or allocation pressure: fail"
  echo "reason: pytest rejected the live prefix-cache isolation or allocate_slots invariant"
  echo 0 > /logs/verifier/reward.txt
  exit 0
fi
