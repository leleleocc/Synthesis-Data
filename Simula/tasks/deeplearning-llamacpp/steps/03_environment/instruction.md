...
This image is a **shared base**, not a task. Phase 04 (Simula plan) and
phase 05 must still be able to emit `generation.max_candidates` diverse
tasks (every requested tag and `oracle_type` the repo can host). Do not
specialize the base toward one scenario, one defect, or one fixture set.
...
- Do not write:
  - `/synthesis/output/candidates/**` (phase 05+)
  - any `tests/`, `test.sh`, golden files, or verifier scaffolds (phase 07)
  - later state files (`04_simula_plan.json`, `05_task_design.json`, ...)
...
- Do not design, preview, or plant a specific later task in this image.
  ...
  single "the" workload. Those belong in phase 05 overlays. The base must
  remain a working, general Harbor agent image.
- **Deliverables first.** After a short read of the inputs, write the real
  files under `/synthesis/output/environment/` (Dockerfile, setup.sh,
  setup-overlay.sh, smoke_test.sh, assets/) **before** any long docker pull,
  scratch compile, or package-version hunt. Proving the repo builds in a
  throwaway container is optional and only allowed **after** those five paths
  exist on disk. A turn that only probes and never writes the environment
  tree fails the verifier (`missing directory: /synthesis/output/environment`).
- Do not spend the phase deliberating in plain text. Each tool round must
  either write/edit a deliverable path, run the sealed `docker build` /
  smoke, or run GATE. Do not loop on "let me think / producing / go".
...
1. Short read of target_spec / profile / gate / repo layout (minutes, not a
   full compile).
2. Land the skeleton, then **immediately** write the environment tree
   (Step 02). Do not open a long probe loop between these two.
3. Fill the state JSON (Step 03) with real values (not the skeleton
   placeholders).
4. `docker build -t synth-env:03` and smoke (Step 04); only then optional
   extra checks inside the built image.
5. GATE.
...
That creates the state file. Replace skeleton values as soon as you know
`workdir`, `data_dir`, `results_dir`, provisional `entrypoints`, and
`network_policy=build_only` when you will `git clone` at build time. The
`build` object lives only in this state file. Do not write a companion
`environment.json`. Leaving `build.failures: ["skeleton: build not yet run"]`
at stop time is a failed phase.
...
and patches belong in phase 05 overlays, not here.
...
Phase 05 knows the task and owns overlays; it adds observation tools then.
...
- Pin every package version. Prefer the repository's real toolchain over a
  generic `python:3.12-slim` toy base. Reasonable pins from the repo README /
  CI / lockfiles are enough; do not block writing files on exhaustive
  `apt-cache` archaeology in a side container.
...
- Skill Example 1 (`FROM python:3.12-slim` plus COPY of a buggy
  ...
  belongs in a phase 05 overlay, never in this shared base.
...
### Step 01: choose topology from tags and domain (short)
...
must be able to host. Cap this step: skim profile entrypoints, root README /
Makefile / CI for the toolchain, confirm `docker info`, run
`ensure-handoff`, then move to Step 02. Do **not** compile the repo in a
scratch container before the environment tree exists.
...
### Step 02: write environment files (mandatory before any long build)

`mkdir -p /synthesis/output/environment/assets` and write all four scripts
plus the Dockerfile in this step. The verifier looks for that directory; if
it is missing, reward is 0 even when a scratch Redis build elsewhere
succeeded. Prefer writing a complete first draft over perfect pins; fix pins
on rebuild after smoke fails.

`/synthesis/output/environment/` is the sealed base. Harbor later builds each
packaged task's `environment/` with that directory as context after phase 09
...
* `setup-overlay.sh`: **no-op in the base** (shebang plus a comment that phase
  05 overlays may replace this file). Still executable. Phase 09 merge lets a
...
Harbor runtime `task.toml` `[environment].network_mode` (phase 05 sets that to
...
Only after Step 02 files exist. This phase is the **only** acceptance gate for
the sealed base. Write logs to `/synthesis/logs/build.log`. Wait until
`docker info` succeeds, then:
...
test -f /synthesis/output/environment/Dockerfile \
  ...
  || { echo "REFUSING_BUILD_MISSING_ENV_TREE"; exit 1; }
...
`data_dir` / `results_dir` work. Record `build.build_seconds` in the state
JSON and clear skeleton `failures`.

Build failure is a content problem to fix under `/synthesis/output/environment/`;
it is not a reason to skip the JSON handoff or to keep probing outside that
tree. After a fix, rebuild and smoke again.

Optional deeper checks (repo `runtest`, extra package probes) run **inside**
`synth-env:03` after smoke passes. They must not replace writing the tree or
the sealed build.
