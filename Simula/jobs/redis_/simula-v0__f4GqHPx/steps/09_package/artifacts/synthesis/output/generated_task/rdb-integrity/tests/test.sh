...
# tests/dump.rdb is a known-valid Redis 7 snapshot the repaired checker must
# accept. /data/dump.rdb is produced at image build and includes AUX ctime /
# used-mem, so it must not be required to match this fixture byte-for-byte.
gold_hash="$(sha256sum "$GOLD" | awk '{print $1}')"
[ "$gold_hash" = "86d891ddbdee4179dbec8672514237dfff8f0f10666f33e95a1123bf89124f54" ] || fail

hdr="$(head -c 9 "$DUMP")"
[ "$hdr" = "REDIS0011" ] || fail
grep -aq 'user:1' "$DUMP" || fail
grep -aq 'alice' "$DUMP" || fail
grep -aq 'cache:flag' "$DUMP" || fail
grep -aq 'stream:events' "$DUMP" || fail

# Intact overlay dump and the known-valid fixture must both be accepted.
set +o pipefail
"$CHK" "$DUMP" >/tmp/c2_check_ok.out 2>/tmp/c2_check_ok.err
chk_rc=$?
"$CHK" "$GOLD" >/tmp/c2_check_gold.out 2>/tmp/c2_check_gold.err
gold_rc=$?
set -o pipefail
if [ "$chk_rc" -eq 0 ]; then
  chk_status=ok
...
[ "$gold_rc" -eq 0 ] || fail
grep -q 'RDB CRC error' /tmp/c2_check_ok.out /tmp/c2_check_ok.err /tmp/c2_check_gold.out /tmp/c2_check_gold.err 2>/dev/null && fail
...
# Loaded from the dump, not SET/PEXPIRE after an expire-unit failure.
loaded="$(info_field persistence rdb_last_load_keys_loaded)"
expired="$(info_field persistence rdb_last_load_keys_expired)"
if is_int "$loaded" && [ "$loaded" -ge 3 ]; then
  :
else
  fail
fi
