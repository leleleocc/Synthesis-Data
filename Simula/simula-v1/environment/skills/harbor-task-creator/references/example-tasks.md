...
Note: Flask is pinned to an exact version. The agent might need Flask to develop the API, so it belongs in the Dockerfile. Bake `pytest==9.1.1`, `pytest-json-ctrf==0.5.2`, and `requests==2.32.3` into the same image when the verifier is inline (or into `tests/Dockerfile` when separate). `test.sh` must not install packages.
...
# pytest==9.1.1, pytest-json-ctrf==0.5.2, and requests==2.32.3 are in the verifier image.
...
- No verify-time installs; pytest and requests are already in the image
...
# pytest, pytest-json-ctrf, psycopg2-binary, and postgresql-client are in the verifier image.
...
Note: Bake `postgresql-client` and `psycopg2-binary==2.9.10` into the verifier image, not `test.sh`. The `PGHOST`, `PGUSER`, `PGPASSWORD`, `PGDATABASE` env vars come from `[verifier].env` in task.toml, so test scripts and psycopg2 can connect to Postgres without hardcoding credentials.
